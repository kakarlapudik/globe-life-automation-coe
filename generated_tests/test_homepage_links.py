"""
Test Case: UC001 - Verify Globe Life Investor Relations Homepage Links
Description: Validate all links on the homepage return HTTP 200 status
"""
...
class TestHomepageLinks:
    """Test all links on Globe Life Investor Relations homepage"""
    ...
    def test_homepage_loads_successfully(self):
        """Verify homepage loads with 200 status"""
        response = self.page.goto(self.BASE_URL)
        assert response.status == 200, f"Homepage failed to load: {response.status}"
        expect(self.page).to_have_url(self.BASE_URL)
    
    def test_extract_all_homepage_links(self):
        ...
        print(f"\n✓ Extracted {len(self.all_links)} links from homepage")
        assert len(self.all_links) > 0, "No links found on homepage"
    
    def test_validate_all_homepage_links(self):
        """Validate all homepage links return 200 status"""
        self.page.goto(self.BASE_URL)
        self.page.wait_for_load_state("networkidle")
        ...
        report = {
            "test": "Homepage Links Validation",
            "url": self.BASE_URL,
        ...
        # Report results
        print(f"\n{'='*60}")
        print(f"Total Links Validated: {validated_count}")
